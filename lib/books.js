...
exports.readBooksConfig = function(options) {
  return utils.readYaml(options.gitbooks || '/etc/gitbook-server/gitbooks.yml');
};

exports.updateBooks = function(books, options) {
  return p.all(_.map(books, function(book) {
    return exports.updateBook(book, options);
  }));
};
...
exports.updateBook = function(book, options) {
  ...
  function generatePostReceiveHook() {
    return utils.loadHandlebarsTemplate(__dirname + '/../templates/post-receive.hbs').then(function(template) {
